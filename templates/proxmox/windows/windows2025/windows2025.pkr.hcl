...
source "proxmox-iso" "windows2025" {
  ...
  template_description = "Windows Server 2025 GitHub Actions self-hosted runner image"

  boot_iso {
    iso_file = var.pmx_windows2025_iso_file
    unmount  = true
    type     = "ide"
    index    = "0"
  }
  ...
  additional_iso_files {
    iso_file         = var.pmx_virtio_iso_file
    unmount          = true
    iso_storage_pool = var.pmx_iso_storage
    type             = "ide"
    index            = "1"
  }
...
build {
  ...
  provisioner "powershell" {
    inline = [
      ...
      "Move-Item '${local.image_folder}\\toolsets\\toolset-2025.json' '${local.image_folder}\\toolset.json'",
  ...
  provisioner "shell-local" {
    inline = ["echo '[image-builder:stage:base-image-config-defender-powershell-wsl2-windows-features-chocolatey] Base image config: Defender, PowerShell, WSL2, Windows features, Chocolatey'"]
  }
  ...
  provisioner "powershell" {
    ...
    scripts = [
      ...
      "${local.windows_root}/scripts/build/Install-WSL2.ps1",
  ...
  provisioner "shell-local" {
    inline = ["echo '[image-builder:stage:install-docker-compose-powershell-core-and-webpi] Install Docker, Compose, PowerShell Core and WebPI'"]
  }
  ...
  provisioner "shell-local" {
    inline = ["echo '[image-builder:stage:install-wix-vs-extensions-azure-cli-chocolatey-packages-java-kotlin-op] Install WiX, VS extensions, Azure CLI, Chocolatey packages, Java, Kotlin, OpenSSL'"]
  }
  ...
  provisioner "shell-local" {
    inline = ["echo '[image-builder:stage:postgresql-windows-updates-shell-and-developer-mode-config-llvm] PostgreSQL, Windows Updates, shell and developer mode config, LLVM'"]
  }
  ...
  provisioner "powershell" {
    ...
    scripts = [
      ...
      "${local.windows_root}/scripts/build/Invoke-Cleanup.ps1",
