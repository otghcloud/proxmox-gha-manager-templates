...
build {
  ...
  provisioner "file" {
    ...
    source      = "${local.ubuntu_root}/toolsets/toolset-2604.json"
  }
  ...
  # The bulk of GitHub's toolset - matches the order used by the real ubuntu-26.04 azure-arm build.
  provisioner "shell" {
  ...
  # Homebrew must install as the unprivileged build user, not root.
  provisioner "shell" {
  ...
  # Pre-stage (but do NOT register) the GitHub Actions runner binary so VMs cloned
  # from this template only need a fresh registration token at deploy time.
  provisioner "shell" {
    inline = [
      "set -e",
      ...
      # Release assets are versioned (e.g. actions-runner-linux-x64-2.321.0.tar.gz) -
      # there is no unversioned filename under releases/latest/download.
  ...
  # Proxmox template hygiene (replaces upstream's Azure-only waagent deprovision step).
  provisioner "shell" {
    inline = [
